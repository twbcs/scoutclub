class Dashboard::Admin::ForumsController < Dashboard::Admin::AdminController
  ...
  def create
    ...
    if @forum.save
      redirect_to(dashboard_admin_forum_types_url, notice: '新增討論板完成')
    else
      render :new
    end
  end
  ...
  def update
    ...
    if @forum.update(forum_params)
      redirect_to(dashboard_admin_forum_types_url, notice: '修改討論板完成')
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    if post < 1
      ...
      @forum.destroy
      redirect_to(dashboard_admin_forum_types_url, notice: '討論板已刪除')
    else
      redirect_to(dashboard_admin_forum_types_url, alert: '板內有文章，無法進行刪除作業')
    end
  end
