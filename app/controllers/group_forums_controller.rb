class GroupForumsController < ApplicationController
  def new
  ...
  def create
    ...
    if @group_forum.save
      redirect_to group_path(@group_forum.group_id), notice: '權限新增完成'
    else
      render :new
    end
  end
  ...
  def destroy
    @group_forum = GroupForum.find(params[:id])
    @group_forum.destroy
    redirect_to group_path(@group_forum.group_id), notice: '權限已刪除'
  end
